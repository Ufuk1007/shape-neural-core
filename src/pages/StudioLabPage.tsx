...
const NeuralCloud = lazy(() => import("@/components/NeuralCloud"));

function SignalCloudLoader({ language }: { language: "de" | "en" }) {
  ...
  return (
    <section id="cloud" ref={anchorRef} className="ss-cloud-loader" aria-label={language === "de" ? "Interaktive Signal Cloud" : "Interactive Signals Cloud"}>
      {shouldLoad ? (
        <Suspense fallback={<div className="ss-cloud-loader__status">{language === "de" ? "SIGNAL CLOUD WIRD GELADEN …" : "LOADING SIGNALS CLOUD …"}</div>}>
          <NeuralCloud variant="studio" language={language} />
        </Suspense>
      ) : <div className="ss-cloud-loader__status">{language === "de" ? "SIGNAL CLOUD" : "SIGNALS CLOUD"}</div>}
    </section>
  );
}
...
const copy = {
  ...
  en: {
    ...
    roleBody: "There is deliberately no second project gallery and no repeat of the Studio narrative here. The cloud retains its function as an exploratory surface — visually connected to the Studio, editorially independent.",
    modes: [
...
export default function StudioLabPage() {
  ...
  return (
    <div className="ss-site ss-site--lab">
      <StudioMeta title="Lab — ShapeNeural" description={t.intro} path="/studio/lab" language={language} />
      ...
      <main>
        ...
        <SignalCloudLoader language={language} />
        ...
        <section className="ss-lab-principle ss-section"><p className="ss-eyebrow">{t.principleLabel}</p><div><h2>{t.principleTitle}</h2><p>{t.principleBody}</p><a href="#cloud">{language === "de" ? "Zur Signal Cloud zurück" : "Return to the Signals Cloud"}<ArrowRight size={16} /></a></div></section>
